...
(function () {
    ...
    hexya.testing = typeof QUnit === 'object';
    hexya.remainingJobs = jobs;
    hexya.__DEBUG__ = {
    ...
    hexya.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === 'string' ? args.shift() : ('__hexya_job' + (jobUID++));
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: hexya.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name
                };
                if (jobs[k].error) {
                ...
                var deps = hexya.__DEBUG__.getDependencies(job.name);
            ...
            missing = hexya.__DEBUG__.getMissingJobs();
            failed = hexya.__DEBUG__.getFailedJobs();
            ...
            if (hexya.debug || failed.length || unloaded.length) {
                ...
                if (hexya.debug && Object.keys(debugJobs).length) {
                    log('Debug:                   ', debugJobs);
                }
            }
        }
        hexya.__DEBUG__.jsModules = {
            missing: missing,
    ...
    hexya.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec).then(
                        function (data) {
                            ...
                            resolve();
                            hexya.processJobs(jobs, services);
