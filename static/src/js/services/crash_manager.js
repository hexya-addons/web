...
var CrashManagerDialog = Dialog.extend({
    xmlDependencies: (Dialog.prototype.xmlDependencies || []).concat(
        ['/static/web/src/xml/crash_manager.xml']
    ),
...
var CrashManager = AbstractService.extend({
    ...
    show_error: function (error) {
        ...
        var dialog = new ErrorDialog(this, {
            title: _.str.capitalize(error.type) || _t("Hexya Error"),
        }, {
...
var RedirectWarningHandler = Widget.extend(ExceptionHandler, {
    ...
    display: function() {
        ...
        new WarningDialog(this, {
            title: _.str.capitalize(error.type) || _t("Hexya Warning"),
            buttons: [
...
core.crash_registry.add('hexya.exceptions.RedirectWarning', RedirectWarningHandler);

function session_expired(cm) {
    return {
        display: function () {
            cm.show_warning({type: _t("Hexya Session Expired"), message: _t("Your Hexya session expired. Please refresh the current web page.")});
        }
    };
}
core.crash_registry.add('hexya.http.SessionExpiredException', session_expired);
