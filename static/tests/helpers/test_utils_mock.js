hexya.define('web.test_utils_mock', function (require) {
"use strict";
 ...
 *   used to generate mock answers for every kind of routes supported by hexya
 * @param {number} [params.debug] if set to true, logs RPCs and uncaught Hexya
 *   events.
...
function addMockEnvironment(widget, params) {
    ...
    if ('config' in params) {
        ...
        if ('debug' in params.config) {
            hexya.debug = params.config.debug;
        }
    }
    ...
    // make sure all Hexya events bubbling up are intercepted
 ...
 * not fire.
 *
 * @param {Widget} widget the target widget (any Hexya widget)
