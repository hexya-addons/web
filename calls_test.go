...
"bin_size":true,"active_test":false}`),
			...
			n, _ := srr[0].Underlying().Get("name")
			So(n, ShouldEqual, "User")
			c, _ := srr[0].Underlying().Get("company_id")
			cID, _ := c.([2]interface{})
			So(cID[0], ShouldEqual, newCompanyID)
			So(cID[1], ShouldEqual, "Company4")
			a, _ := srr[0].Underlying().Get("active")
			So(a, ShouldBeTrue)
		})
...
"search_default_customer":true,"params":{"action":"base_action_partner_form"}}`),
		...
		Convey("Read on Company", func() {
			...
			n, _ := rr[0].Underlying().Get("name")
			So(n, ShouldEqual, "Company4")
			id, _ := rr[0].Underlying().Get("id")
			So(id, ShouldEqual, newCompanyID)
			dn, _ := rr[0].Underlying().Get("display_name")
